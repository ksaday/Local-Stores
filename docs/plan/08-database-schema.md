...
- **plans** — SaaS plans. `code PK`, `name`, `price_cents`, `interval`, `trial_days`, `platform_fee_bps`, `limits jsonb`, `stripe_price_id`. **v2.0 ships exactly one row** — `STANDARD`, 4900 cents, monthly, 30 trial days, **`platform_fee_bps = 0`** ([§18.5a](18-final-recommendations.md#185a-pricing--49storemonth)). The table and the `plan_code` FK exist so a second tier is later a row rather than a migration; no plan-gating or upgrade logic ships in v2.0
- **`platform_fee_bps` is deliberately expressible on `stores` as well as `plans`** — not redundancy. BBA publicly commits to taking no cut of sales ([§18.6](18-final-recommendations.md#186-transaction-fee--zero-and-stated-publicly)); if that ever changes, existing stores must be grandfathered at 0 permanently, which requires a per-store value that no future plan change can override. `stores.platform_fee_bps IS NULL` means "inherit the plan"; an explicit `0` means "zero, forever, regardless of plan"
